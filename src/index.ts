...
// 설정 스키마 (선택사항 - API 키 등 사용자 설정이 필요한 경우)
export const configSchema = z.object({
    hfToken: z.string().optional().describe('Hugging Face API 토큰 (이미지 생성용)')
})
...
// Smithery 배포용 createServer 함수 export
    ...
    server.resource(
        ...
        async (uri) => {
            const serverInfo = {
                name: SERVER_NAME,
                version: SERVER_VERSION,
                tools: ['greeting', 'calc', 'time', 'generate-image'],
                description: '인사말, 계산기, 시간 조회, 이미지 생성 기능을 제공하는 MCP 서버입니다.'
            }
    ...
    server.tool(
        ...
        async ({ prompt }) => {
            try {
                ...
                if (!token) {
                    return {
                        content: [{ type: 'text', text: '오류: Hugging Face API 토큰이 설정되지 않았습니다' }]
                    }
                }
...
if (isDirectRun && typeof process !== 'undefined') {
